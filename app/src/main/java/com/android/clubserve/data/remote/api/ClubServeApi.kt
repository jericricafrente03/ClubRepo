...
import com.android.clubserve.data.remote.interceptor.CurlLoggerInterceptor

interface ClubServeApi {
    suspend fun getHomeData(): List<JsonElement>

    companion object {
        fun create(): ClubServeApi {
            return ClubServeApiImpl(
                HttpClient(OkHttp) {
                    engine {
...
class ClubServeApiImpl(private val client: HttpClient) : ClubServeApi {
    override suspend fun getHomeData(): List<JsonElement> {
